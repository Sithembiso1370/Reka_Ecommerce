...
exports.saveSupplierOrder = async (req, res) => {
  try {
    ...
    if(req.file){
      ...
      const enquiry = await SupplierOrder.create({
        url,
        public_id,
        ...fields,
        createdBy,
...
exports.getSupplierOrders =async (req, res) => {
  try {
    const enquiries = await SupplierOrder.find({});
    const sortedEnquiries = enquiries.sort((a, b) => b.createdAt - a.createdAt);
    res.send(sortedEnquiries);
  } catch (error) {
    res.status(400).send('Error while getting list of SupplierOrders. Try again later.');
  }
}
    
 
exports.getOneSupplierOrder = async (req, res) => {
  try {
    const enquiry = await SupplierOrder.findById(req.params.id);
    if (!enquiry) return res.status(404).send('SupplierOrder not found');
    res.json(enquiry);
  } catch (error) {
...
exports.updateOneSupplierOrder = async (req, res) => {
  try {
    const enquiry = await SupplierOrder.findById(req.params.id);
    if (!enquiry) return res.status(404).send('SupplierOrder not found');
    if (req.file) {
    ...
    await enquiry.save();
    res.send('SupplierOrder updated successfully.');
  } catch (error) {
    res.status(500).send(error);
  }
}
...
exports.deleteOneSupplierOrder = async (req, res) => {
  try {
    // Get the enquiry ID from the request parameters
    const { id } = req.params;

    // Find the enquiry in the database by its ID
    const enquiry = await SupplierOrder.findById(id);

    // If no enquiry was found, return a 404 response
    if (!enquiry) {
      return res.status(404).send('SupplierOrder not found');
    }
    ...
    res.send('SupplierOrder deleted successfully.');
  } catch (error) {
    // If there was an error, return a 500 response
    console.log('Delete error:', error);
    res.status(500).send('Error while deleting SupplierOrder. Try again later.');
  }
}

//  Supplier orders controller and model same fields