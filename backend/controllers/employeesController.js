...
exports.saveEmployee = async (req, res) => {
  try {
    ...
    if(req.file){
      ...
      const enquiry = await Employee.create({
        url,
        public_id,
        ...fields,
        createdBy,
...
exports.getEmployees =async (req, res) => {
  try {
    const enquiries = await Employee.find({});
    const sortedEnquiries = enquiries.sort((a, b) => b.createdAt - a.createdAt);
    res.send(sortedEnquiries);
  } catch (error) {
    res.status(400).send('Error while getting list of Employees Try again later.');
  }
}
...
exports.getOneEmployee = async (req, res) => {
  try {
    const enquiry = await Employee.findById(req.params.id);
    if (!enquiry) return res.status(404).send('Employee not found');
    res.json(enquiry);
  } catch (error) {
...
exports.updateOneEmployee = async (req, res) => {
  try {
    const enquiry = await Employee.findById(req.params.id);
    if (!enquiry) return res.status(404).send('Employee not found');
    if (req.file) {
...
exports.deleteOneEmployee = async (req, res) => {
  try {
    ...
    const enquiry = await Employee.findById(id);
    ...
    res.status(500).send('Error while deleting enquiry. Try again later.');
  }
}
