...
const CustomerInvoice = require('../models/customerInvoicesModel');
...
exports.saveCustomerInvoice = async (req, res) => {
  try {
...
exports.getCustomerInvoices =async (req, res) => {
  try {
    ...
    const sortedCustomerInvoices = invoices.sort((a, b) => b.createdAt - a.createdAt);
    res.send(sortedCustomerInvoices);
  } catch (error) {
    res.status(400).send('Error while getting list of Customer Invoices. Try again later.');
  }
}
    
 
exports.getOneCustomerInvoice = async (req, res) => {
  try {
    const enquiry = await CustomerInvoice.findById(req.params.id);
    if (!enquiry) return res.status(404).send('Customer Invoice not found');
    res.json(enquiry);
  } catch (error) {
...
exports.updateOneCustomerInvoice = async (req, res) => {
  try {
    const enquiry = await CustomerInvoice.findById(req.params.id);
    if (!enquiry) return res.status(404).send('Customer Invoice not found');
    if (req.file) {
    ...
    await enquiry.save();
    res.send('Customer Invoice updated successfully.');
  } catch (error) {
    res.status(500).send(error);
  }
}
...
exports.deleteOneCustomerInvoice = async (req, res) => {
  try {
    // Get the enquiry ID from the request parameters
    const { id } = req.params;

    // Find the enquiry in the database by its ID
    const enquiry = await CustomerInvoice.findById(id);

    // If no enquiry was found, return a 404 response
    if (!enquiry) {
      return res.status(404).send('Customer Invoice not found');
    }
    ...
    res.send('Customer Invoice deleted successfully.');
  } catch (error) {
    // If there was an error, return a 500 response
    console.log('Delete error:', error);
    res.status(500).send('Error while deleting Customer Invoice. Try again later.');
  }
