...
const createRepair = async (req, res) => {
  try {
    ...
    if(req.file){
      ...
      const enquiry = await Repair.create({
        url,
        public_id,
        ...fields,
        createdBy,
...
const getAllRepairs = async (req, res) => {
  try {
    const enquiries = await Repair.find({});
    const sortedEnquiries = enquiries.sort((a, b) => b.createdAt - a.createdAt);
    res.send(sortedEnquiries);
  } catch (error) {
    res.status(400).send('Error while getting list of Repairs. Try again later.');
  }
};
...
const getRepairById = async (req, res) => {
  try {
    const enquiry = await Repair.findById(req.params.id);
    if (!enquiry) return res.status(404).send('Repair not found');
    res.json(enquiry);
  } catch (error) {
...
const updateRepairById = async (req, res) => {
  try {
    ...
    await enquiry.save();
    res.send('Repair updated successfully.');
  } catch (error) {
    res.status(500).send(error);
  }
};
...
const deleteRepairById = async (req, res) => {
  try {
    ...
    const enquiry = await Repair.findById(id);
    ...
    res.send('Repair deleted successfully.');
  } catch (error) {
    // If there was an error, return a 500 response
    console.log('Delete error:', error);
    res.status(500).send('Error while deleting Repair. Try again later.');
  }
};
