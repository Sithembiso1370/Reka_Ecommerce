...
exports.saveCustomer = async (req, res) => {
  try {
    ...
    if(req.file){
      ...
      const enquiry = await Customers.create({
        url,
        public_id,
        ...fields,
        createdBy,
...
exports.getCustomers =async (req, res) => {
  try {
    ...
    const enquiries = await Customers.find({});
    const sortedEnquiries = enquiries.sort((a, b) => b.createdAt - a.createdAt);
    res.send(sortedEnquiries);
  } catch (error) {
    res.status(400).send('Error while getting list of Customers. Try again later.');
  }
}
...
exports.getOneCustomer = async (req, res) => {
  try {
    const enquiry = await Customers.findById(req.params.id);
    if (!enquiry) return res.status(404).send('Customers not found');
    res.json(enquiry);
  } catch (error) {
...
exports.deleteOneCustomer = async (req, res) => {
  try {
    ...
    const enquiry = await Customers.findById(id);
    ...
    res.status(500).send('Error while deleting Customers. Try again later.');
  }
}
