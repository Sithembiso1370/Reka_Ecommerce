...
exports.saveSupplier = async (req, res) => {
  try {
    ...
    if(req.file){
      ...
      const enquiry = await Suppliers.create({
        url,
        public_id,
        ...fields,
        createdBy,
...
exports.getSuppliers =async (req, res) => {
  try {
    const enquiries = await Suppliers.find({});
...
exports.getOneSupplier = async (req, res) => {
  try {
    const enquiry = await Suppliers.findById(req.params.id);
    if (!enquiry) return res.status(404).send('Suppliers not found');
    res.json(enquiry);
  } catch (error) {
...
exports.updateOneSupplier = async (req, res) => {
  try {
    const enquiry = await Suppliers.findById(req.params.id);
    if (!enquiry) return res.status(404).send('Suppliers not found');
    if (req.file) {
    ...
    await enquiry.save();
    res.send('Suppliers updated successfully.');
  } catch (error) {
    res.status(500).send(error);
  }
}
...
exports.deleteOneSupplier = async (req, res) => {
  try {
    ...
    res.send('Suppliers deleted successfully.');
  } catch (error) {
    // If there was an error, return a 500 response
    console.log('Delete error:', error);
    res.status(500).send('Error while deleting Suppliers. Try again later.');
  }
}
